...
let paletteIndex=0;
function applyPalette(i){const[bg,fg,a1,a2,mut,ring]=PALETTES[i%PALETTES.length];const r=document.documentElement.style;r.setProperty('--bg',bg);r.setProperty('--fg',fg);r.setProperty('--accent',a1);r.setProperty('--accent2',a2);r.setProperty('--muted',mut);r.setProperty('--ring',ring);}
...
// ===== Starfield + Spiral (canvas-only so no seams) =====
...
const STARS=[];
function seedStars(){STARS.length=0;const count=Math.floor((innerWidth*innerHeight)/9000);for(let i=0;i<count;i++){STARS.push({x:Math.random()*SW,y:Math.random()*SH,r:Math.random()*1.2+.2,a:Math.random()*Math.PI*2,tw:Math.random()*0.8+0.2});}}
...
const spiral=document.getElementById('spiral'), ctx=spiral.getContext('2d');
...
function draw(now){drawStars(); ctx.clearRect(0,0,W,H); const wobble=30*DPR, cX=CX+wobble*mouse.x, cY=CY+wobble*mouse.y;
  ctx.lineWidth=Math.max(1,DPR*0.7); ctx.strokeStyle=getComputedStyle(document.documentElement).getPropertyValue('--ring');
  ctx.beginPath(); let theta=0, first=true, spin=now*0.00012;
  while(theta<26){const a=2,b=0.15,rad=a*Math.exp(b*theta)*3.5*DPR; const x=cX+Math.cos(theta+spin)*rad, y=cY+Math.sin(theta+spin)*rad; if(first){ctx.moveTo(x,y); first=false;} else {ctx.lineTo(x,y);} theta+=0.08;}
  ctx.stroke();
  const a1=getComputedStyle(document.documentElement).getPropertyValue('--accent'); const a2=getComputedStyle(document.documentElement).getPropertyValue('--accent2');
...
function hubCard(id){
  const el=document.createElement('article'); el.className='hcard'; el.tabIndex=0;
  el.innerHTML=`<h2>${SECTION_INFO[id].title}</h2><p>${SECTION_INFO[id].desc}</p>`;
  el.onclick=()=>location.hash=`#/section/${id}`;
  el.onkeypress=e=>{ if(e.key==='Enter') location.hash=`#/section/${id}`; };
  return el;
}
function renderHub(){
  hubEl.hidden=false; sectionsEl.innerHTML=''; navRow.hidden=true;
...
function cardFor(t){
  ...
  el.dataset.slug=t.slug; el.innerHTML=`<h2 class="card-title">${t.title}</h2><p class="card-sub">${t.subtitle||''}</p>`;
  el.onclick=()=>location.hash=`#/term/${t.slug}`;
  el.onkeypress=e=>{ if(e.key==='Enter') location.hash=`#/term/${t.slug}`; };
  return el;
}
function renderSection(tag){
  hubEl.hidden=true; sectionsEl.innerHTML=''; navRow.hidden=false;
  ...
  TERMS.filter(t => (t.tags||[])[0]===tag).forEach(t => g.appendChild(cardFor(t)));
  sec.appendChild(g); sectionsEl.appendChild(sec);
}
function renderTermContent(t){
  const c=t.content||{};
  const list = (arr, label)=> Array.isArray(arr)&&arr.length ? `<h3>${label}</h3><ul>${arr.map(x=>`<li>${x}</li>`).join('')}</ul>` : '';
  const cite = (arr)=> Array.isArray(arr)&&arr.length ? `<h3>References</h3><ul>${arr.map(x=>`<li>${x}</li>`).join('')}</ul>` : '';
  const tags = (t.tags||[]).map(x=>`<span class="label">${x}</span>`).join('');
  return `
    <h2>${t.title}</h2>
    ${t.subtitle?`<p><em>${t.subtitle}</em></p>`:''}
    ${c.definition?`<h3>Definition</h3><p>${c.definition}</p>`:''}
    ${list(c.looks_like,'What it looks like')}
    ${list(c.overlaps,'Overlaps')}
    ${c.importance?`<h3>Importance</h3><p>${c.importance}</p>`:''}
    ${cite(c.citations)}
...
function showDetail(slug){
  ...
  if(t && t.content){ termContent.innerHTML = renderTermContent(t); }
  else {
