...
function fitSky(){ SW = sky.width = Math.floor(innerWidth * SDPR); SH = sky.height = Math.floor(innerHeight * SDPR);
  sky.style.width = innerWidth + 'px'; sky.style.height = innerHeight + 'px'; drawStars(); }
const STARS = [];
function seedStars(){ STARS.length = 0; const count = Math.floor((innerWidth*innerHeight) / 9000);
  for(let i=0;i<count;i++){ STARS.push({ x: Math.random()*SW, y: Math.random()*SH, r: Math.random()*1.2 + 0.2, a: Math.random()*Math.PI*2, tw: Math.random()*0.8 + 0.2 }); } }
function drawStars(){ sctx.clearRect(0,0,SW,SH); sctx.fillStyle = 'white';
  for(const s of STARS){ const twinkle = 0.85 + 0.15*Math.sin(s.a += s.tw*0.03); sctx.globalAlpha = 0.5 + 0.5*twinkle;
...
pauseBtn.addEventListener('click', togglePause); paletteBtn.addEventListener('click', cyclePalette);
...
function draw(now){ if(paused) return; drawStars(); ctx.clearRect(0,0,W,H);
  const wobbleR = 30 * DPR; const cX = CX + wobbleR * mouse.x; const cY = CY + wobbleR * mouse.y;
  ctx.lineWidth = Math.max(1, DPR*0.7); ctx.strokeStyle = getComputedStyle(document.documentElement).getPropertyValue('--ring');
  ctx.beginPath(); let theta = 0; let first = true; const spin = reverse * now * 0.00012;
  while(theta < 26){ const rad = a * Math.exp(b * theta) * 3.5 * DPR; const x = cX + Math.cos(theta + spin) * rad; const y = cY + Math.sin(theta + spin) * rad;
    if(first){ ctx.moveTo(x,y); first = false; } else { ctx.lineTo(x,y); } theta += 0.08; } ctx.stroke();
  const accent = getComputedStyle(document.documentElement).getPropertyValue('--accent'); const accent2 = getComputedStyle(document.documentElement).getPropertyValue('--accent2');
  theta = 0; let i = 0; while(i < MAX_POINTS){ const rad = a * Math.exp(b * theta) * 3.5 * DPR; const x = cX + Math.cos(theta + spin) * rad; const y = cY + Math.sin(theta + spin) * rad;
    if(!(x < -50 || x > W+50 || y < -50 || y > H+50)){ ctx.beginPath(); ctx.fillStyle = (i % 2) ? accent : accent2;
      const size = 0.5 + 1.2 * Math.pow(i/MAX_POINTS, 0.45); ctx.globalAlpha = 0.4 + 0.4*Math.sin(now*0.003 + i);
      ctx.arc(x, y, size*DPR, 0, Math.PI*2); ctx.fill(); ctx.globalAlpha = 1; } theta += thetaStep; i++; }
  requestAnimationFrame(draw);
}
const prefersReduced = matchMedia('(prefers-reduced-motion: reduce)').matches; if(!prefersReduced){ requestAnimationFrame(draw); }

/* -------- Grid + Router -------- */
const gridEl = document.getElementById('cardGrid'); const gridView = document.getElementById('gridView');
const detailView = document.getElementById('detailView'); const termContent = document.getElementById('termContent'); const backBtn = document.getElementById('backBtn');
...
function showGrid(){ detailView.hidden = true; gridView.hidden = false; termContent.innerHTML = ''; if(location.hash !== '#/'){ history.pushState(null, '', '#/'); } }
function showDetail(slug){
  gridView.hidden = true; detailView.hidden = false; termContent.innerHTML = `<p style="opacity:.7">Loading…</p>`;
...
function route(hash){ const m = hash.match(/^#\/term\/([\w\-]+)/); if(m){ showDetail(m[1]); } else { showGrid(); } }
addEventListener('hashchange', ()=> route(location.hash));
backBtn.addEventListener('click', (e)=>{ e.preventDefault(); showGrid(); });
addEventListener('keydown', (e)=>{ if(e.key === 'Escape'){ showGrid(); } });
