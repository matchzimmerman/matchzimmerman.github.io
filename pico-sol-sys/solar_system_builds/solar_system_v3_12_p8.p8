...
version 42
__lua__
-- solar_system_v3.12.p8
-- comet reset, orbit speed bars, orbit crossing glow
...
function _update60()
    ...
    update_comet()
    if btnp(4) then reset_comet() end
    orbit_pulse += 0.02
end

function update_planets()
    for p in all(planets) do
        p.angle += p.speed * 0.98
...
function update_comet()
    ...
    if comet.x > 140 or comet.y > 140 then reset_comet() end
end
