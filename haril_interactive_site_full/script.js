...
// Data
async function loadData(){ const res=await fetch('data.json'); const json=await res.json(); state.data=json; renderAll(); applyQueryParams(); routeFromHash(); }
loadData();

function renderAll(){ renderCards(); renderRelations(); renderTelemetry(); highlightView(); renderViz(); setupPages(); }
...
function renderCards(){
  ...
  tiers.forEach(t=>{
    ...
    card.innerHTML=`
      ...
      <div class="links"><a href="#/tier/${t.tier}">Open details</a></div>
    `;
...
function applyQueryParams(){
  ...
  const view=params.get('view'); if(view) { state.view=view; highlightView(); renderViz(); location.hash=`#/${state.view}`; }
  const tier=params.get('tier'); if(tier) openTierModal(parseInt(tier));
  const theme=params.get('theme'); if(theme) document.body.setAttribute('data-theme', theme);
  const tour=params.get('tour'); if(tour==='1') openTour(true);
}

// Tour
const tourDlg=qs('#tourDlg'); qs('#tourDlg .close').addEventListener('click',()=>tourDlg.close()); qs('#tourDone').addEventListener('click',()=>{ localStorage.setItem('haril_tour_done','1'); tourDlg.close(); });
qs('#tour').addEventListener('click', ()=>openTour(true));
function openTour(force=false){ if(!force && localStorage.getItem('haril_tour_done')==='1') return; tourDlg.showModal(); }
